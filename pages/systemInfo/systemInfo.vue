<template>
	<view class="container">
		<view class="content">
			<scroll-view scroll-y="true" :style="{height:'calc(100vh)'}" @scrolltoupper="upper"
				@scrolltolower="lower">
						...
						<view class="jydata">
							<block v-for="(itennr,index) in item.arr" :key="index">
								<view class="item">
									<view class="itemxx">
										<view class="xxle">
											<text>{{itennr.title}}</text>
											<text v-if="itennr.description">{{itennr.description}}</text>
										</view>
										<view class="xxri iconfont icon-a-lujing448"></view>
									</view>
									<view class="itemTime">{{itennr.createTime}}</view>
								</view>
							</block>
						</view>
					</view>
