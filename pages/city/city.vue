<template>
	<view class="contanier">
		<view class="content">
			<view class="top" id="test">
				<view class="ss">
					<view class="sscon">
						<text class="ssico iconfont icon-a-lujing293"></text>
						<input placeholder="中文/拼音/首字母" placeholder-class="pyclass" @input='bindSarchInput' />
					</view>
				</view>
				<view class="cs">
					<view class="cstop">定位/最近访问</view>
					<view class="csdata csdata1 font26 color3">
						<text class="iconfont icon-a-lujing453"></text>{{cityname}}
					</view>
				</view>
				<view class="cs">
					<view class="cstop">热门城市</view>
					<view class="csdata csdata2">
						<view class="item" data-name="北京" @click="clickCity">北京</view>
						<view class="item" data-name="上海" @click="clickCity">上海</view>
						<view class="item" data-name="广州" @click="clickCity">广州</view>
						<view class="item" data-name="成都" @click="clickCity">成都</view>
						<view class="item" data-name="深圳" @click="clickCity">深圳</view>
						<view class="item" data-name="宜城" @click="clickCity">宜城</view>
					</view>
				</view>
			</view>
			<view class='a-z' style="top: 142px">
				<view v-for="(item,index) in cityAZ" :key="unique">
					<view :class="azIndex==index?'azho':''" :data-index="index" :data-id='item.cityName'
						@click='bindAZ'>
						{{item.cityName}}
					</view>
				</view>
			</view>
			<view class='city-item-content' :style="{'margin-top':hei}">
				<view v-for="(item,index) in cityResults" :key="unique">
					<view v-if="item.cityPinYin.length > 1" class='city-item' :data-cityname='item.cityName'
						@click='citySelected'>
						{{item.cityName}}
					</view>
					<view v-else class='city-item-A-Z' :data-cityname='item.cityName'>
						{{item.cityName}}
					</view>
				</view>
			</view>
